...
interface SettingsAdminState {
  ...
  setHomeSections: (
    sections:
      { random_books: boolean; top_books: boolean } | ((prev: any) => any),
  ) => void;
...
const initialProxyAuth = {
  enabled: false,
  header_names: ["X-Forwarded-User", "Remote-User", "X-Forwarded-Email"],
...
export const useSettingsAdminStore = create<SettingsAdminState>((set) => ({
  ...
  setSite: (site) =>
    set((state) => ({
      site: typeof site === "function" ? site(state.site) : site,
    })),
  setServerUrl: (serverUrl) => set({ serverUrl }),
  setSidebarItems: (sidebarItems) =>
    set((state) => ({
      sidebarItems:
        typeof sidebarItems === "function"
  ...
  setPasswordResetEnabled: (passwordResetEnabled) =>
    set({ passwordResetEnabled }),
  ...
  setHardcoverClientSecret: (hardcoverClientSecret) =>
    set({ hardcoverClientSecret }),
  setAutoEnrich: (autoEnrich) => set({ autoEnrich }),
  setWebpCover: (webpCover) => set({ webpCover }),
  setLimits: (limits) => set({ limits }),
  setGuestMode: (guestMode) => set({ guestMode }),
  setGuestLibraryIds: (guestLibraryIds) =>
    set((state) => ({
      guestLibraryIds:
        typeof guestLibraryIds === "function"
