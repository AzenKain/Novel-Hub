...
export function useKoboSetupQuery(enabled = true) {
  return useQuery<KoboSetup | null>({
    ...
    queryFn: async () => {
      ...
      if (!res.status)
        throw new Error(res.message || "Failed to load Kobo endpoint");
  ...
  return useMutation({
    mutationFn: async () => {
      ...
      if (!res.status || !res.data)
        throw new Error(res.message || "Failed to regenerate Kobo endpoint");
      return res.data;
    },
    onSuccess: (data) => {
...
export function useRevokeKoboSetupMutation() {
  ...
  return useMutation({
    mutationFn: async () => {
      ...
      if (!res.status)
        throw new Error(res.message || "Failed to revoke Kobo endpoint");
      return res;
    },
    onSuccess: () => {
