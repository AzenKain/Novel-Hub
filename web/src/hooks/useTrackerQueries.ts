...
export function useTrackerConnectionsQuery(enabled = true) {
  return useQuery<TrackerConnection[]>({
    ...
    queryFn: async () => {
      ...
      if (!res.status)
        throw new Error(res.message || "Failed to load tracker connections");
      return res.data ?? [];
    },
    enabled,
    staleTime: 30_000,
...
export function useConnectTrackerMutation() {
  ...
  return useMutation({
    mutationFn: async (input: ConnectTrackerInput) => {
      const res = await trackerService.connectTracker(
        input.provider,
        input.access_token,
      );
      if (!res.status)
        throw new Error(res.message || "Failed to connect tracker");
      return res;
    },
    onSuccess: () => {
  ...
  return useMutation<TrackerSearchResult, Error, string>({
    mutationFn: async (title: string) => {
      ...
      if (!res.status || !res.data)
        throw new Error(res.message || "No AniList match found");
      return res.data;
    },
  });
}

export function useTrackerReadingProgressQuery(
  book_id: string,
  enabled = true,
) {
  return useQuery({
...
export function useMapBookTrackerMutation() {
  ...
  return useMutation({
    mutationFn: async (input: MapTrackerInput) => {
      const res = await trackerService.mapBookTracker(
        input.book_id,
        input.provider,
        input.external_series_id,
      );
      if (!res.status)
        throw new Error(res.message || "Failed to map book tracker");
      return res;
    },
  });
}
...
export function useSyncTrackerProgressMutation() {
  return useMutation({
    mutationFn: async (input: SyncProgressInput) => {
      const res = await trackerService.syncProgress(
        input.book_id,
        input.title,
        input.progress,
      );
      if (!res.status)
        throw new Error(res.message || "Failed to sync reading progress");
      return res;
    },
  });
}
...
export function useConnectHardcoverMutation() {
  return useMutation({
    mutationFn: async () => {
      ...
      if (!res.status || !res.data?.authorize_url)
        throw new Error(res.message || "Failed to start Hardcover connect");
...
export function useSyncHardcoverMutation() {
  return useMutation<void, Error, { book_id: string; progress: number }>({
    mutationFn: async (input) => {
      const res = await trackerService.syncHardcoverProgress(
        input.book_id,
        input.progress,
      );
      if (!res.status)
        throw new Error(res.message || "Failed to sync Hardcover progress");
    },
  });
}
...
export function useExportHighlightsToReadwiseMutation() {
  return useMutation<{ exported: number }, Error, string>({
    mutationFn: async (book_id: string) => {
      ...
      if (!res.status || !res.data)
        throw new Error(res.message || "Failed to export highlights");
      return res.data;
    },
  });
}
