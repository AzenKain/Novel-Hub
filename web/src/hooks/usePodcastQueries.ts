  ...
  return useQuery<Podcast[]>({
    ...
    queryFn: async () => {
      ...
      if (!res.status)
        throw new Error(
          res.message ||
            i18n.t("podcasts.load_failed", "Failed to load podcasts"),
        );
      return res.data || [];
    },
  });
  ...
  return useQuery<PodcastEpisode[]>({
    ...
    queryFn: async () => {
      ...
      if (!res.status)
        throw new Error(
          res.message ||
            i18n.t("podcasts.load_failed", "Failed to load episodes"),
        );
    ...
    enabled: !!podcastId,
    refetchInterval: () =>
      usePodcastDownloadStore.getState().hasActiveDownloads(podcastId)
        ? 2000
        : false,
  ...
  return useMutation({
    mutationFn: async (input: SubscribePodcastInput) => {
      ...
      if (!res.status)
        throw new Error(
          res.message ||
            i18n.t("podcasts.subscribe_failed", "Failed to subscribe"),
        );
      return res.data;
    },
    onSuccess: () => {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      if (!res.status)
        throw new Error(
          res.message ||
            i18n.t("podcasts.update_failed", "Failed to update podcast"),
        );
      return res.data;
    },
    onSuccess: () => {
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      ...
      if (!res.status)
        throw new Error(
          res.message ||
            i18n.t("podcasts.delete_failed", "Failed to delete podcast"),
        );
    },
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: ["podcasts"] });
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      ...
      if (!res.status)
        throw new Error(
          res.message ||
            i18n.t("podcasts.refresh_failed", "Failed to refresh podcast"),
        );
      return res.data;
    },
    onSuccess: () => {
...
export function useDownloadEpisodeMutation(podcastId: string) {
  ...
  return useMutation({
    mutationFn: async (params: string | DownloadEpisodeParams) => {
      ...
      const episodeTitle =
        typeof params === "string" ? undefined : params.episodeTitle;
      ...
      if (!res.status)
        throw new Error(
          res.message ||
            i18n.t("podcasts.download_failed", "Failed to enqueue download"),
        );
      return res.data;
    },
    onSuccess: () => {
