...
export const useHighlights = (
  ...
  const highlightsQuery = useQuery<Highlight[]>({
    ...
    queryFn: async () => {
      ...
      if (!res.status)
        throw new Error(res.message || "Failed to fetch highlights");
  ...
  const addMutation = useMutation({
    mutationFn: async ({
      ...
      if (!chapter_id || !book_id)
        throw new Error("Missing chapter_id or book_id");
      const res = await highlightService.createHighlight(
        book_id,
        chapter_id,
        text_content,
        start_index,
      ...
      if (!res.status)
        throw new Error(res.message || "Failed to create highlight");
      return res.data;
    },
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: async ({
      ...
      if (!res.status)
        throw new Error(res.message || "Failed to update highlight");
      return res.data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["highlights"] });
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      ...
      if (!res.status)
        throw new Error(res.message || "Failed to delete highlight");
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["highlights"] });
    },
  ...
  const addHighlight = async (
    ...
    try {
      ...
      toast.error(
        i18n.t(
          "reader.highlight_create_failed",
  ...
  const updateHighlight = async (id: string, color: string, note?: string) => {
    try {
      ...
      toast.error(
        i18n.t(
          "reader.highlight_update_failed",
  ...
  const removeHighlight = async (id: string) => {
    try {
      ...
      toast.error(
        i18n.t(
          "reader.highlight_delete_failed",
