...
import {
  MIN_DOUBLE_PAGE_WIDTH,
  READER_CONTENT_MEASURE,
  READER_PAGE_GAP,
  getSideTapRatio,
} from "@/constants";
...
const ReaderWorkspaceInner = () => {
  ...
  const [quoteModalImageUrl, setQuoteModalImageUrl] = useState<
    string | undefined
  >(undefined);
  ...
  const pendingSearchMatchRef = useRef<{
    query: string;
    snippet?: string;
  } | null>(null);
  ...
  const [offlineRawUrl, setOfflineRawUrl] = useState<string | undefined>(
    undefined,
  );
  ...
  const allowTTS = hasPermission(
    user,
    "book.tts",
    book?.library_id,
    guestPerms,
  );
  const allowHighlights = hasPermission(
    user,
    "book.highlight",
    book?.library_id,
    guestPerms,
  );
  const currentChapterExists = Boolean(
    currentChapter && chapters.some((c) => c.id === currentChapter.id),
  );
  const { data: readerETA } = useReaderETAQuery(
    user && hasPermission(user, "user.stats.read") ? book_id : undefined,
  );
  const { highlights, addHighlight, updateHighlight, removeHighlight } =
    useHighlights(
      book?.id || "",
      currentChapter?.id,
      allowHighlights && currentChapterExists,
    );
  ...
  const [activeImageTarget, setActiveImageTarget] =
    useState<ActiveImageTarget | null>(null);
  ...
  const handleSaveImageBookmark = (
    bm: Omit<ImageBookmark, "id" | "created_at">,
  ) => {
      ...
      setTimeout(() => {
        ...
        if (container) {
          ...
          const matched = imgs.find(
            (img) =>
              img.src === bm.image_url ||
              img.getAttribute("src") === bm.image_url,
          );
          if (matched) {
            const isPaged =
            ...
            if (isPaged && pagedContainer.clientWidth > 0) {
              ...
              const pageIndex = Math.max(
                0,
                Math.floor(relativeLeft / scrollStep),
              );
  ...
  const {
    isScrolling: autoScrollActive,
    toggleScroll: onToggleAutoScroll,
    updateSpeed,
  ...
  const doublePageWidth =
    pageFrameWidth > 0 ? Math.floor((pageFrameWidth - READER_PAGE_GAP) / 2) : 0;
  const canUseDoubleMode =
    !isMobileScreen &&
    (pageFrameWidth === 0
      ? typeof window !== "undefined" && window.innerWidth >= 768
      : doublePageWidth >= MIN_DOUBLE_PAGE_WIDTH);
  const effectiveReadingMode =
    readingMode === "double" && !canUseDoubleMode ? "single" : readingMode;

  const scrollLayout =
    effectiveReadingMode === "scroll" || effectiveReadingMode === "webtoon";
  const activeFile = file_id
    ? book?.files?.find((f) => f.id === file_id)
    : book?.files?.[0];
  ...
  const isPdf = !!(
    ...
    currentChapter?.content_path?.toLowerCase().endsWith(".pdf")
  );
  ...
  useEffect(() => {
    ...
    void fetch(rawFileUrl, { method: "HEAD", credentials: "include" })
      .then((res) =>
        res.ok ? undefined : Promise.reject(new Error("unreachable")),
      )
  ...
  const defaultPadding =
    typeof window !== "undefined" && window.innerWidth < 640 ? 32 : 160;
  const rawFrameWidth =
    pageFrameWidth > 0
      ? pageFrameWidth
      : typeof window !== "undefined"
        ? Math.max(0, Math.min(window.innerWidth - defaultPadding, maxWidth))
        : 920;
  const pageWidth = scrollLayout
    ...
    : Math.max(
        1,
        Math.floor(
          (rawFrameWidth - READER_PAGE_GAP * (visiblePages - 1)) / visiblePages,
        ),
      );
  ...
  useEffect(() => {
    ...
    const handlePointerDownOutside = (event: PointerEvent) => {
      ...
      if (
        ...
        target.closest('.modal, [role="dialog"], [data-reader-modal="true"]')
      )
        return;
      setSidebarOpen(false);
    };
  ...
  const computeProgressPercent = (): number => {
    ...
    return Math.min(
      100,
      Math.round(((chapterPosition + fraction) / chapters.length) * 100),
    );
  };
    ...
    const bootstrap = async () => {
      try {
        const [res, progressRes] = await Promise.allSettled([
          ...
          user
            ? featureService.getReadingProgress(book_id)
        ...
        const loaded = offline
          ...
          : res.status === "fulfilled" && res.value.status
            ? res.value.data
            : null;

        if (loaded) {
          setBook(loaded.book);
          const sorted = [...loaded.chapters].sort(
            (a, b) => a.chapter_index - b.chapter_index,
          );
          setChapters(sorted);
          if (sorted.length > 0) {
            let targetChapter =
              sorted.find((ch) => !isNavOnlyChapter(ch.title)) || sorted[0];
              ...
              if (
                ...
                if (found && !isNavOnlyChapter(found.title)) {
                  ...
                  if (
                    ...
                    progress.location_cfi.startsWith("epubcfi(")
                  ) {
                    location_cfi = progress.location_cfi;
                  } else if (
                    progress.location_type === "scroll" &&
                    progress.location_cfi
                  ) {
                    location_cfi = `scroll:${progress.location_cfi}`;
                  } else if (
                    progress.location_type === "page" &&
                    progress.location_cfi
                  ) {
                    location_cfi = `page:${progress.location_cfi}`;
                  } else if (
                    progress.location_type === "audio" &&
                    progress.location_cfi
                  ) {
                ...
                const guestHist = useGuestStore
                  .getState()
                  .getReadingHistory()
                  .find((h) => h.book_id === book_id);
                if (guestHist) {
                  const found = sorted.find(
                    (ch) => ch.id === guestHist.chapter_id,
                  );
                  if (found && !isNavOnlyChapter(found.title)) {
                    targetChapter = found;
                  }
  ...
  const loadChapter = async (chapter: Chapter) => {
    ...
    try {
      ...
      ttsAutoContinueNextChapterRef.current = false;
      const stored = await offlineStore
        .getChapter(book_id, chapter.id)
        .catch(() => undefined);
      if (stored) {
      ...
      setHtmlContent(
        `<div class='text-error p-4'>${t("offline.chapter_unavailable", "This chapter is not available offline. Save the book for offline reading while you are connected.")}</div>`,
      );
    }
  };
  ...
  const invalidateProgressQueries = useCallback(() => {
    ...
    void queryClient.invalidateQueries({
      queryKey: ["trackerReadingProgress"],
    });
    void queryClient.invalidateQueries({
  ...
  const reportProgressFailure = useCallback(
    (error: unknown) => {
      ...
      toast.warning(
        t(
          "reader.progress_sync_failed",
          "Reading progress is not being saved right now",
        ),
      );
    },
    [t],
  ...
  const getVisibleCfi = (): string => {
    ...
    const children = Array.from(
      container.querySelectorAll(
        "p, h1, h2, h3, h4, h5, h6, li, figure, img, div",
      ),
    );
    const containerRect = container.getBoundingClientRect();
    ...
    for (const child of children) {
      if (
        child.tagName === "DIV" &&
        child.querySelector("p, h1, h2, h3, h4, h5, h6, li, figure, img")
      ) {
        continue;
      }
    ...
    return generateCfi(
      ...
      targetNode,
      0,
      currentChapterIndex >= 0 ? currentChapterIndex : 0,
      currentChapter.id,
    );
  };
  ...
  const handleScroll = () => {
    if (!scrollLayout || !contentRef.current || !currentChapter || !book_id)
      return;
  ...
  const nextInReadList =
    !hasNextChapter && readListNext?.has_next ? readListNext.book : undefined;
  const nextInSeries =
    !hasNextChapter && !nextInReadList && seriesContext?.next
      ? seriesContext.next
      : undefined;
  ...
  const nextTooltip = useMemo(() => {
    ...
    if (nextInReadList)
      return t("reader.readlist_next", "Next: {{title}}", {
        title: nextInReadList.title,
      });
    if (nextInSeries)
      return t("reader.readlist_next", "Next: {{title}}", {
  ...
  return (
    <div
      className={`reader-ui reader-theme-${theme} drawer h-screen overflow-hidden ${readerBg}`}
    >
      <input
        id="reader-drawer"
        type="checkbox"
      ...
      <div className="drawer-content flex flex-col h-screen overflow-hidden relative">
        <ReaderTopBar
          ...
          onOpenSearch={
            publicSettings?.enable_in_book_search
              ? () => {
                  lastFocusedControlRef.current =
                    document.activeElement as HTMLElement | null;
                  setSearchOpen(true);
            ...
            >
              ...
              >
                {isPdf ? (
                  ...
                  <div className="flex-1 w-full h-full">
                    <AudioPlayer
                      ...
                      cover_url={
                        book.cover_url || `${API_BASE}/books/${book.id}/cover`
                      }
                      chapters={audiobookChapters?.map((c) => ({
                        title: c.title,
                        start_sec: c.start_sec,
                        end_sec: c.end_sec,
                      }))}
                      initialTime={
                        pendingFragmentRef.current?.startsWith("audio:")
                          ? parseFloat(pendingFragmentRef.current.slice(6))
                          : 0
                      }
                      ...
                      onTimeUpdate={(time) => {
                        ...
                        const progress_percent =
                          chapterPosition >= 0
                            ? Math.round(
                                ((chapterPosition + 1) / chapters.length) * 100,
                              )
                            : 0;
                    ...
                    {readerETA && readerETA.eta_minutes > 0 && (
                      <p className="text-center text-xs text-(--reader-ui-muted) mt-2">
                        {t("reader.eta_left", "~{{minutes}} min left", {
                          minutes: Math.max(1, readerETA.eta_minutes),
                        })}
                      </p>
                    )}
                  </div>
                  ...
                  <ComicReader
                    ...
                    onPrevChapter={
                      canGoPrev ? () => handlePrev(true) : undefined
                    }
                ...
                {!isPdf && !isAudio && scrollLayout && nextInReadList && (
                  ...
                  >
                    {t("reader.readlist_next", "Next: {{title}}", {
                      title: nextInReadList.title,
                    })}
                  </button>
                )}
              ...
              {!isPdf && !isAudio && !scrollLayout && htmlContent && (
                <ReaderPageControls
                  ...
                  canGoNext={
                    isComicFormat
                      ? comicPage + comicStep < comicTotalPages || canGoNext
                      : true
                  }
                  onPrev={
                    isComicFormat
      ...
      <ReaderSidebar
        ...
        onUpdateHighlight={
          allowHighlights
            ? (id, color, note) => void updateHighlight(id, color, note)
            : undefined
        }
        onDeleteHighlight={
          allowHighlights ? (id) => void removeHighlight(id) : undefined
        }
          ...
          <ReaderInBookSearch
            ...
            onSelectResult={(res) => {
              const ch = chapters.find(
                (c) =>
                  c.id === res.chapter_id ||
                  (res.chapter_index !== undefined &&
                    c.chapter_index === res.chapter_index) ||
