...
export function Books() {
  ...
  const store = useBookAdminStore(
    useShallow((state) => ({
      ...
      bookFiles: state.bookFiles,
      uploadingBookFiles: state.uploadingBookFiles,
      coverTab: state.coverTab,
      epubImages: state.epubImages,
      ...
      uploadLibraryId: state.uploadLibraryId,
      uploading: state.uploading,
      showLibraryModal: state.showLibraryModal,
      newLibraryName: state.newLibraryName,
      bookToDelete: state.bookToDelete,
      libraryToDelete: state.libraryToDelete,
      setSearch: state.setSearch,
      setSelectedLibraryId: state.setSelectedLibraryId,
      setSearchSource: state.setSearchSource,
      setOnlineSearchQuery: state.setOnlineSearchQuery,
      setCoverTab: state.setCoverTab,
      setLinkUrl: state.setLinkUrl,
      setFormData: state.setFormData,
      setShowUploadModal: state.setShowUploadModal,
      setUploadLibraryId: state.setUploadLibraryId,
      setShowLibraryModal: state.setShowLibraryModal,
      setNewLibraryName: state.setNewLibraryName,
      setEditingBook: state.setEditingBook,
      setSearchResults: state.setSearchResults,
      setBookToDelete: state.setBookToDelete,
      setLibraryToDelete: state.setLibraryToDelete,
      openEditModal: state.openEditModal,
      handleSearchOnline: state.handleSearchOnline,
      handleSelectResult: state.handleSelectResult,
      handleSelectEpubImage: state.handleSelectEpubImage,
      handleImageUpload: state.handleImageUpload,
      handleLinkUpload: state.handleLinkUpload,
      handleEditSubmit: state.handleEditSubmit,
      handleUploadBookFiles: state.handleUploadBookFiles,
      handleUploadFiles: state.handleUploadFiles,
      deleteBook: state.deleteBook,
      archiveBook: state.archiveBook,
  ...
  const books = useMemo(
    () => booksPages?.pages.flatMap((page) => page.data || []) ?? [],
    [booksPages],
  );
  ...
  const allSelectedAudioFiles = useMemo(() => {
    return selectedBooks
      .flatMap((b) => b.files || [])
      .filter((f) => MERGEABLE_FORMATS.has(f.format.toLowerCase()));
  }, [selectedBooks]);

  const canBulkMerge = allSelectedAudioFiles.length >= 2;

  const canBulkConvert = selectedBooks.some(
  ...
  const isAllSelected =
    books.length > 0 && books.every((b) => selectedBookIds.includes(b.id));
  ...
  return (
    <div className="flex flex-col h-full bg-base-100">
          ...
          <div className="flex flex-col xl:flex-row items-stretch xl:items-center justify-between gap-3 sm:gap-4 mb-6">
            ...
            <div className="flex flex-1 flex-col sm:flex-row items-stretch sm:items-center gap-2.5 sm:gap-3 min-w-0">
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                >
                  <option value="">
                    {t("admin.all_libraries", "All Libraries")}
                  </option>
                  {libraries.map((lib) => (
            ...
            <div className="rounded-2xl border border-dashed border-base-300 bg-base-100 p-12 sm:p-16 text-center flex flex-col items-center justify-center gap-3 shadow-xs">
              ...
              <div>
                ...
                <p className="text-xs sm:text-sm text-base-content/60 mt-1 max-w-sm">
                  {t(
                    "admin.no_books_hint",
                    "Configure a library in the system and upload EPUB files.",
                  )}
                </p>
              </div>
            </div>
                  ...
                  {books.map((book) => {
                    ...
                    return (
                      <BookCard
                        ...
                        selected={selIndex !== -1}
                        selectionIndex={
                          selIndex !== -1 ? selIndex + 1 : undefined
                        }
                  ...
                  <table className="table table-zebra w-full">
                    <thead>
                      <tr className="bg-base-200/50 text-base-content/70">
                        ...
                        <th className="w-[34%] max-w-85">
                          {t("admin.books", "Books")}
                        </th>
                        <th className="w-[16%] max-w-45">
                          {t("admin.author", "Author")}
                        </th>
                        <th className="w-[26%] max-w-70">
                          {t("admin.series", "Series")}
                        </th>
                    ...
                    <tbody>
                      {books.map((book) => {
                        ...
                        const meta = book.metadata_json
                          ? parseMetadata(book.metadata_json)
                          : {};
                        ...
                        return (
                          <tr
                            key={book.id}
                            className={`hover ${isSelected ? "bg-primary/5" : ""}`}
                          >
                            <td
                            ...
                            >
                              ...
                              >
                                <div
                                  className={`h-5.5 min-w-5.5 rounded-md flex items-center justify-center border transition-all select-none ${
                                    isSelected
                              ...
                              >
                                <div className="w-10 h-14 bg-base-300 rounded shadow-xs overflow-hidden shrink-0 relative flex items-center justify-center">
                                  {book.cover_url ? (
                                    ...
                                    <span className="text-[9px] font-bold opacity-40 text-center px-1 uppercase">
                                      {t("book.novel_label")}
                                    </span>
                                  )}
                                </div>
                                <div className="min-w-0 flex-1">
                                  ...
                                  {book.files && book.files.length > 0 && (
                                    <div className="text-xs opacity-50 flex items-center gap-1.5 mt-0.5">
                                      <span className="badge badge-ghost badge-xs uppercase font-mono">
                                        {book.files[0].format}
                                      </span>
                                      <span>
                                        {formatFileSize(
                                          book.files[0].size_bytes,
                                        )}
                                      </span>
                                    </div>
                                  )}
          ...
          <div className="modal-box w-11/12 max-w-[96vw] 2xl:max-w-[1700px] h-[95vh] max-h-[96vh] bg-base-100 shadow-2xl p-0 overflow-hidden flex flex-col rounded-2xl">
            <header className="px-6 py-4 border-b border-base-200 bg-base-200/30 flex items-center justify-between shrink-0">
              <div>
                <h3 className="text-xl font-black">
                  {t("admin.metadata", "Metadata")}
                </h3>
                <p className="text-xs opacity-50 mt-0.5 font-mono truncate max-w-sm">
                  {editingBook?.id}
                </p>
              </div>
              <button
              ...
              <div className="grid grid-cols-1 lg:grid-cols-[340px_1fr] gap-0 min-h-0">
                <div className="flex flex-col gap-4 p-6 border-r border-base-200 bg-base-200/10">
                  <span className="text-xs font-bold text-primary uppercase tracking-wider">
                    {t("admin.cover_image", "Cover Image")}
                  </span>
                  ...
                  <div className="flex gap-1 flex-nowrap">
                    <button
                      type="button"
                      onClick={() => setCoverTab("book")}
                      className={`btn btn-xs flex-1 gap-1 whitespace-nowrap rounded-lg ${coverTab === "book" ? "btn-primary" : "btn-ghost"}`}
                    >
                    ...
                    <button
                      ...
                      className={`btn btn-xs flex-1 gap-1 whitespace-nowrap rounded-lg ${coverTab === "upload" ? "btn-primary" : "btn-ghost"}`}
                    >
                    ...
                    <button
                      ...
                      className={`btn btn-xs flex-1 gap-1 whitespace-nowrap rounded-lg ${coverTab === "link" ? "btn-primary" : "btn-ghost"}`}
                    >
                            ...
                            return (
                              ...
                              >
                                <img
                                  ...
                                  alt={fileName}
                                  loading="lazy"
                                  className="w-9 h-12 object-cover rounded-lg bg-base-200 border border-base-200 shrink-0"
                                />
                    ...
                    {coverTab === "link" && (
                      ...
                      >
                        <span className="text-[11px] font-bold opacity-60">
                          {t("admin.paste_image_url", "Paste image URL")}
                        </span>
                        <input
                          type="url"
                  ...
                  <div className="border-t border-base-200 pt-4">
                    ...
                    <label
                      className={`mb-3 flex min-h-20 cursor-pointer flex-col items-center justify-center gap-1.5 rounded-xl border-2 border-dashed border-base-300 p-3 text-center transition-colors hover:border-primary/30 hover:bg-primary/5 ${uploadingBookFiles ? "pointer-events-none opacity-60" : ""}`}
                    >
                      <input
                        type="file"
                        multiple
                      ...
                      <span className="text-xs font-medium opacity-70">
                        {uploadingBookFiles
                          ...
                          : t("admin.add_formats", "Add formats")}
                      </span>
                      <span className="text-[10px] opacity-40">
                    ...
                    <div className="flex max-h-40 flex-col gap-1.5 overflow-y-auto pr-1">
                      {bookFiles.length > 0 ? (
                        bookFiles.map((file: BookFile) => (
                          <div
                            key={file.id}
                            className="flex min-w-0 items-center gap-2 rounded-lg border border-base-200 bg-base-100/70 p-2"
                          >
                            ...
                            <div className="min-w-0 flex-1">
                              ...
                              <div className="text-[10px] uppercase text-base-content/45">
                                {file.format || "file"} ·{" "}
                        ...
                        <div className="rounded-lg bg-base-200/40 px-3 py-2 text-center text-xs text-base-content/45">
                          {t("admin.no_files_attached")}
                        </div>
                      )}
                    </div>
                  </div>
                ...
                <div className="flex flex-col gap-6 p-6 overflow-y-auto">
                  ...
                  >
                    <div className="flex flex-col gap-1.5 w-full">
                      <label className="text-xs font-bold text-base-content/80 pl-1">
                        {t("book.title", "Title")}
                      </label>
                      <input
                        type="text"
                        required
                        className="input input-bordered input-md text-sm rounded-xl w-full font-medium"
                    ...
                    <div className="flex flex-col gap-1.5 w-full">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setFormData({ author: e.target.value })
                        }
                      />
                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row gap-4">
                      ...
                      <div className="flex flex-col gap-1.5 sm:w-28 shrink-0">
                        ...
                        <input
                          type="text"
                          className="input input-bordered input-md text-sm rounded-xl w-full font-mono text-center font-medium"
                          value={formData.series_index}
                          onChange={(e) =>
                            setFormData({ series_index: e.target.value })
                          }
                        />
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div className="flex flex-col gap-1.5 w-full">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setFormData({ publisher: e.target.value })
                          }
                        />
                      </div>
                      <div className="flex flex-col gap-1.5 w-full">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div className="flex flex-col gap-1.5 w-full">
                        ...
                        <input
                          type="text"
                          className="input input-bordered input-md text-sm rounded-xl w-full font-mono font-medium"
                      ...
                      <div className="flex flex-col gap-1.5 w-full">
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            setFormData({ age_rating: e.target.value })
                          }
                            ...
                            {formData.subjects.map((sub, sIdx) => (
                              ...
                              >
                                ...
                                <button
                                  type="button"
                                  onClick={() => {
                                    const next = formData.subjects.filter(
                                      (_, i) => i !== sIdx,
                                    );
                    ...
                    <div className="flex flex-col gap-1.5 w-full">
                      ...
                      <textarea
                        rows={5}
                        className="textarea textarea-bordered textarea-md w-full leading-relaxed resize-y text-sm rounded-xl"
                    ...
                    {editingBook?.metadata_json && (
                      <div className="flex flex-col gap-1.5 w-full mt-2">
                        <label className="text-xs font-bold text-base-content/80 pl-1">
                          {t("book.identifiers", "Identifiers")}
                        </label>
                          ...
                          {(() => {
                            try {
                              ...
                              return meta.identifier
                                ? JSON.stringify(meta.identifier, null, 2)
            ...
            <footer className="px-6 py-4 border-t border-base-200 bg-base-200/30 flex justify-end gap-3 shrink-0">
              ...
              <button
                ...
                form="metadata-form"
                disabled={submitting}
                className="btn btn-primary btn-sm sm:btn-md px-6 gap-1.5 rounded-xl font-bold shadow-lg shadow-primary/20"
              >
                {submitting ? (
          ...
          onImport={(path, library_id) => {
            calibreImportMutation.mutate(
              { path, library_id: library_id || undefined },
              {
                onSuccess: (data) => {
                  toast.success(
                    t("admin.calibre_import_success", {
      ...
      <ManageLibrariesModal
        ...
        onRename={(id, name) =>
          updateLibraryMutation.mutate(
            ...
            {
              onSuccess: () =>
                toast.success(t("admin.library_renamed", "Library renamed")),
            },
          )
        }
      ...
      <DeleteConfirmModal
        ...
        message={
          <>
            ...
            <strong>{bookToDelete?.title}</strong>? This will permanently delete
            the EPUB file and all associated reading data from the server.
          </>
        }
        ...
        message={
          <div className="space-y-2.5">
            ...
            <div className="rounded-xl bg-error/10 border border-error/20 p-3 text-xs text-error font-medium leading-relaxed">
              ⚠️ <strong>{t("admin.danger_warning", "Danger warning:")}</strong>{" "}
              {t(
                "admin.delete_library_danger_desc",
                "This will permanently delete all books, chapters, files and reading data belonging to this library. This data CANNOT be recovered!",
              )}
            </div>
          </div>
        }
          ...
          if (libraryToDelete) {
            deleteLibraryMutation.mutate(libraryToDelete.id, {
              onSuccess: () => {
                toast.success(
                  t(
                    "admin.library_delete_queued",
                    "Library deletion request received. The process is running in the background.",
                  ),
                );
                if (selectedLibraryId === libraryToDelete.id)
                  setSelectedLibraryId("");
                if (uploadLibraryId === libraryToDelete.id)
                  setUploadLibraryId("");
              },
            });
      ...
      <DeleteConfirmModal
        ...
        message={t(
          "admin.confirm_delete_file",
          "Are you sure you want to delete this file format?",
        )}
