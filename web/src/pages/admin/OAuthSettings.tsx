...
import { ArrowLeft, Check, Key, Loader2, Save, Shield, HelpCircle } from "lucide-react";
...
export function OAuthSettings() {
  ...
  const { data: adminSettings, isLoading: settingsLoading, refetch } = useAdminSettingsQuery();
  ...
  useEffect(() => {
    if (adminSettings?.oauth) {
      ...
      setOidcScopes(oidc.scopes ? oidc.scopes.join(", ") : "openid, profile, email");
    }
  }, [adminSettings]);
    ...
    <div className="flex flex-col h-full bg-base-100 font-sans">
      ...
      <header className="px-4 py-5 sm:px-6 lg:px-8 lg:py-6 border-b border-base-200 flex items-center justify-between bg-base-100/50 backdrop-blur-xl sticky top-0 z-10">
        <div className="flex items-center gap-3">
          <Link to="/admin/settings" className="btn btn-ghost btn-circle btn-sm">
            <ArrowLeft className="h-5 w-5" />
          </Link>
          <div>
            <h1 className="text-2xl font-bold tracking-tight">{t("settings.oauth.title", "OAuth & OIDC Settings")}</h1>
            <p className="text-sm text-base-content/60 mt-1">{t("settings.oauth.subtitle", "Configure external identity providers for social and enterprise single sign-on")}</p>
          </div>
        </div>
      </header>
        ...
        <div className="max-w-4xl mx-auto space-y-6">
            ...
            <div className="card-body p-5">
              <div className="flex items-center justify-between border-b border-base-200/60 pb-3 mb-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  >
                    {isSaving("google") ? <Loader2 className="h-3 w-3 animate-spin" /> : <Save className="h-3.5 w-3.5" />}
                    {t("admin.save", "Save")}
                  </button>
                </div>
              </div>
          ...
          <div className="card bg-base-100 border border-base-200 shadow-sm overflow-hidden transition-all duration-300 hover:shadow-md">
            <div className="card-body p-5">
              <div className="flex items-center justify-between border-b border-base-200/60 pb-3 mb-4">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-slate-500/10 flex items-center justify-center text-slate-800 dark:text-slate-100 shrink-0">
                    <svg className="h-5 w-5" viewBox="0 0 24 24" fill="currentColor">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button
                    ...
                    className="btn btn-primary btn-sm gap-1"
                  >
                    {isSaving("github") ? <Loader2 className="h-3 w-3 animate-spin" /> : <Save className="h-3.5 w-3.5" />}
                    {t("admin.save", "Save")}
                  </button>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between border-b border-base-200/60 pb-3 mb-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button
                    ...
                    className="btn btn-primary btn-sm gap-1"
                  >
                    {isSaving("discord") ? <Loader2 className="h-3 w-3 animate-spin" /> : <Save className="h-3.5 w-3.5" />}
                    {t("admin.save", "Save")}
                  </button>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between border-b border-base-200/60 pb-3 mb-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button
                    ...
                    className="btn btn-primary btn-sm gap-1"
                  >
                    {isSaving("oidc") ? <Loader2 className="h-3 w-3 animate-spin" /> : <Save className="h-3.5 w-3.5" />}
                    {t("admin.save", "Save")}
                  </button>
                </div>
              </div>
