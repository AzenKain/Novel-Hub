...
export const Duplicates: React.FC = () => {
  ...
  const openKeepOneOnly = (
    keepFileId: string,
    allFiles: { file_id: string }[],
  ) => {
    const toDeleteFileIds = allFiles
      .map((f) => f.file_id)
      .filter((id) => id !== keepFileId);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      if (confirmState.type === "single") {
        ...
        toast.success(
          t("common.success", "Selected files deleted successfully"),
        );
        ...
        toast.success(
          t("common.success", "Duplicate copies removed successfully"),
        );
      }
      void refetch();
    } catch (err) {
    ...
    if (confirmState.type === "selected") {
      return {
        ...
        message: (
          <span>
            {t(
              "admin.confirm_delete_selected",
              `Are you sure you want to delete ${confirmState.file_ids.length} selected file(s)?`,
            )}
          </span>
        ),
      };
          ...
          {duplicateGroups.length > 0 && (
            <div className="flex flex-wrap items-center justify-between gap-3 bg-base-100 p-3 px-4 rounded-xl border border-base-200 shadow-sm">
              <div className="flex items-center gap-2">
                <button
                  onClick={
                    selectedFileIds.size > 0 ? deselectAll : selectAllDuplicates
                  }
                ...
                >
                  ...
                  {selectedFileIds.size > 0
                    ...
                    : t(
                        "admin.select_all_duplicates",
                        "Select All Duplicates (Keep 1/Group)",
                      )}
                </button>
                {selectedFileIds.size > 0 && (
              ...
              {selectedFileIds.size > 0 && (
                ...
                >
                  ...
                  {t("admin.delete_selected", "Delete Selected Files")} (
          ...
          {loading ? (
            ...
            <div className="rounded-2xl border border-dashed border-base-300 bg-base-100 p-12 sm:p-16 text-center flex flex-col items-center justify-center gap-3 shadow-xs">
              ...
              <div>
                <h3 className="text-base sm:text-lg font-bold text-base-content">
                  {t("library.no_duplicates", "No Duplicate Files Found")}
                </h3>
                <p className="text-xs sm:text-sm text-base-content/60 mt-1 max-w-sm">
                  {t(
                    "admin.no_duplicates_desc",
                    "All files in your library are unique. There are no SHA-256 hash collisions.",
                  )}
                </p>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-6">
              {duplicateGroups.map((group) => (
                <div
                  key={group.hash}
                  className="bg-base-100 border border-base-200 p-5 rounded-2xl shadow-sm flex flex-col gap-4"
                >
                  {/* Group Header */}
                      ...
                      return (
                        ...
                        >
                            ...
                            <div className="flex flex-col min-w-0 flex-1 gap-1">
                              <div className="flex items-center gap-2 flex-wrap">
                                <span className="font-bold text-sm text-base-content truncate">
                                  {file.book_title ||
                                    t("common.untitled", "Untitled Book")}
                                </span>
                                {idx === 0 && (
                          ...
                          <div className="flex items-center gap-2 shrink-0 self-end sm:self-center">
                            <button
                              onClick={() =>
                                openKeepOneOnly(file.file_id, group.files || [])
                              }
                              disabled={
                                isDeleting || (group.files?.length || 0) <= 1
                              }
                              className="btn btn-outline btn-xs gap-1 font-normal"
                              title={t(
                                "admin.confirm_keep_one",
                                "Keep this file and delete duplicate copies?",
                              )}
                            >
