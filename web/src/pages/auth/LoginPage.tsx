    ...
    <div className="min-h-screen bg-base-100 flex flex-col font-sans">
      ...
      <div className="flex-1 flex items-center justify-center p-4">
        ...
        <div className="card-body">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            {(loginMutation.error || urlError) && (
              <div className="alert alert-error mt-2 py-2 text-sm rounded-lg flex items-center gap-2">
                <span>
                  {urlError || (loginMutation.error instanceof Error
          ...
          {settings?.oauth?.providers && settings.oauth.providers.some(p => p.enabled) && (
              ...
              <div className="flex flex-col gap-2">
                {settings.oauth.providers
                  ...
                  .map((p) => {
                    ...
                    return (
                      <a key={p.id} href={href} className="btn btn-outline btn-sm h-10 w-full gap-2 border-base-300 font-semibold normal-case">
                        {icon}
                        {t("auth.sign_in_with_provider", `Sign in with ${p.display_name}`, { provider: p.display_name })}
                      </a>
                    );
                  })}
              </div>
