...
function TopList({ items, label }: { items?: NameCount[]; label: string }) {
  ...
  return (
    <div>
      <h3 className="text-xs font-bold uppercase tracking-wider text-base-content/50 mb-2">{label}</h3>
      <div className="flex flex-col gap-1.5">
        {items.slice(0, 6).map((item) => (
          <div key={item.name} className="flex items-center gap-2 text-sm">
            <span className="w-32 truncate shrink-0 text-base-content/80">{item.name}</span>
            ...
            <span className="text-xs text-base-content/50 w-8 text-right shrink-0">{item.count}</span>
          </div>
        ))}
      </div>
    </div>
...
export const ReadingAnalyticsPage: React.FC = () => {
  ...
  const { totalWords, activeDays, todayWords, weekBars } = useMemo(() => {
    ...
    return { totalWords: words, activeDays: days, todayWords: today, weekBars: bars };
  }, [heatmapData]);
      ...
      <div className="flex-1 container mx-auto p-4 sm:p-6 max-w-5xl flex flex-col gap-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="bg-base-100 border border-base-200 p-4 rounded-2xl shadow-sm flex items-center gap-3">
            ...
            <div>
              <div className="text-2xl font-black">{summary?.current_streak_days ?? 0} {t('analytics.days', 'days')}</div>
              <div className="text-xs text-base-content/60">
                {t('analytics.current_streak', 'Day Streak')} · {t('analytics.longest_streak', 'Best: {{count}} days', { count: summary?.longest_streak_days ?? 0 })}
              </div>
            </div>
          </div>
          ...
          <div className="bg-base-100 border border-base-200 p-4 rounded-2xl shadow-sm flex items-center gap-3">
            ...
            <div>
              <div className="text-2xl font-black">{summary ? summary.total_minutes.toLocaleString() : `~${Math.round(totalWords / 250)}`}</div>
              <div className="text-xs text-base-content/60">{t('analytics.total_minutes', 'Minutes Read')}</div>
            </div>
          </div>
        ...
        <div className="bg-base-100 border border-base-200 p-6 rounded-2xl shadow-sm">
          <h2 className="text-lg font-bold mb-4">{t('analytics.this_week', 'This Week')}</h2>
          <div className="flex items-end gap-2 h-24">
            {weekBars.map((bar, i) => (
              <div key={bar.label} className="flex-1 flex flex-col items-center gap-1">
                <div className="w-full rounded-t-lg bg-primary/20" style={{ height: `${Math.max(3, Math.round((bar.words / weekMax) * 100))}%`, minHeight: 4 }} />
                <span className="text-[10px] text-base-content/50">{bar.label}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Library Breakdown */}
        <div className="bg-base-100 border border-base-200 p-6 rounded-2xl shadow-sm">
          <h2 className="text-lg font-bold mb-4">{t('analytics.library_breakdown', 'Library Breakdown')}</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <TopList items={breakdown?.formats} label={t('analytics.top_formats', 'Top Formats')} />
            <TopList items={breakdown?.authors} label={t('analytics.top_authors', 'Top Authors')} />
            <TopList items={breakdown?.tags} label={t('analytics.top_tags', 'Top Genres')} />
            <TopList items={breakdown?.languages} label={t('analytics.top_languages', 'Top Languages')} />
          </div>
          {breakdown && breakdown.avg_speed_wpm > 0 && (
            <p className="mt-3 text-xs text-base-content/60">
              {t('analytics.avg_speed', 'Avg listening speed: {{wpm}} wpm', { wpm: Math.round(breakdown.avg_speed_wpm) })}
            </p>
          )}
          {breakdown?.listening && breakdown.listening.length > 0 && (
            <div className="mt-6">
              <div className="flex items-center gap-2 mb-2">
                ...
                <h3 className="text-xs font-bold uppercase tracking-wider text-base-content/50">{t('analytics.listening_history', 'Listening by Month')}</h3>
              </div>
              <div className="flex items-end gap-2 h-24">
                {breakdown.listening.slice(-12).map((m) => (
                  <div key={m.month} className="flex-1 flex flex-col items-center gap-1">
                    <div className="w-full rounded-t-lg bg-accent/30" style={{ height: `${Math.max(4, Math.round((m.hours / Math.max(1, ...breakdown.listening.map((l) => l.hours))) * 100))}%`, minHeight: 4 }} />
                    <span className="text-[10px] text-base-content/50">{m.month.slice(5)}-{m.month.slice(2, 4)}</span>
                  </div>
                ))}
              </div>
            </div>
          )}
          {breakdown && breakdown.formats.length === 0 && breakdown.tags.length === 0 && (
            <div className="text-xs text-base-content/50 py-4 text-center">
              {t('analytics.no_data', 'No data yet')}
            </div>
          )}
