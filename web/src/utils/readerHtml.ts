...
export const isNavOnlyChapter = (title?: string | null) => {
  ...
  return /^\s*(mục lục|table of contents|contents|目次|목차)\s*$/i.test(
    title.trim(),
  );
};
...
export const sanitizeReaderHtml = (html: string) => {
  ...
  const clean = DOMPurify.sanitize(stripped, {
    ...
    FORBID_ATTR: ["srcdoc"],
    ALLOWED_URI_REGEXP:
      /^(?:(?:(?:f|ht)tps?|mailto|tel|callto|cid|xmpp|blob|data):|[^a-z]|[a-z+.-]+(?:[^a-z+.-:]|$))/i,
  });
  ...
  if (typeof document !== "undefined") {
    ...
    doc
      .querySelectorAll('[align="justify" i], [style*="justify" i]')
      .forEach((el) => {
