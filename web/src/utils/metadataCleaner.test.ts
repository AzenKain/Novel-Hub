...
describe('metadataCleaner tests', () => {
  describe('stripBrackets', () => {
    it('removes square bracket tags from titles', () => {
      expect(stripBrackets('[TangThuVien] Solo Leveling [VIP] [Full]')).toBe('Solo Leveling');
  ...
  describe('stripParentheses', () => {
    it('removes round bracket notes from titles', () => {
      expect(stripParentheses('Lord of the Mysteries (Bản Dịch Chuẩn) (Full)')).toBe('Lord of the Mysteries');
  ...
  describe('toTitleCase', () => {
    it('formats titles into Title Case with minor words handling', () => {
      ...
      expect(toTitleCase('the catcher in the rye')).toBe('The Catcher in the Rye');
    });
  });
  ...
  describe('applyCustomRegex', () => {
    it('replaces regex patterns properly', () => {
      expect(applyCustomRegex('Chương 123: Hồi Kết', '^Chương \\d+:\\s*', '')).toBe('Hồi Kết');
      expect(applyCustomRegex('Chapter 01 - Prologue', '^Chapter \\d+\\s*-\\s*', '')).toBe('Prologue');
    });
  });
