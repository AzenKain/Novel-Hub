...
export async function translateText(
  ...
  try {
    ...
    console.warn(
      "[Translation] Google engine failed, falling back to MyMemory",
      err,
    );
  }
  ...
  try {
    const myMemoryLang =
      targetLang === "zh-CN" || targetLang === "zh-TW"
        ? "zh"
        : targetLang.split("-")[0];
    const langPair =
      sourceLang === "auto"
        ? `autodetect|${myMemoryLang}`
        : `${sourceLang}|${myMemoryLang}`;
    const url = `https:
      langPair
    )}`;
    ...
    console.warn(
      "[Translation] MyMemory engine failed, falling back to Lingva",
      err,
    );
  }
