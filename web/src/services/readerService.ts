...
export const readerService = {
  async getBootstrap(
    book_id: string,
    file_id?: string,
  ): Promise<CommonResponse<BootstrapResponse>> {
    ...
    try {
      ...
      if (axios.isAxiosError(error) && error.response)
        return error.response.data as CommonResponse<BootstrapResponse>;
      throw error;
    }
  },
  ...
  async getChapterHtml(
    ...
    try {
      const res = await api.get(
        `/reader/${book_id}/chapter/${encodeURIComponent(chapter_id)}${query}`,
        {
          responseType: "text",
        },
      );
  ...
  async searchInBook(
    book_id: string,
    query: string,
  ): Promise<CommonResponse<SearchSnippet[]>> {
    try {
      ...
      if (axios.isAxiosError(error) && error.response)
        return error.response.data as CommonResponse<SearchSnippet[]>;
      throw error;
    }
  },

  async syncReadingSession(
    book_id: string,
    duration: number,
    words: number,
  ...
  async getReadingHeatmap(): Promise<
    CommonResponse<Record<string, { duration: number; words: number }>>
  > {
    try {
  ...
  async getReaderETA(book_id: string): Promise<CommonResponse<ReadingETA>> {
    try {
      const res = await api.get(
        `/reader/stats/eta/${encodeURIComponent(book_id)}`,
      );
      return res.data;
    } catch (error) {
      if (axios.isAxiosError(error) && error.response)
  ...
  async getReadingGoal(): Promise<CommonResponse<ReadingGoal>> {
    try {
      ...
      if (axios.isAxiosError(error) && error.response)
        return error.response.data as CommonResponse<ReadingGoal>;
      throw error;
    }
  },
  ...
  ): Promise<CommonResponse<ReadingGoal>> {
    try {
      const res = await api.put("/reader/goals/", {
        target_words_per_day,
        target_books_per_year,
      });
      return res.data;
    } catch (error) {
      if (axios.isAxiosError(error) && error.response)
