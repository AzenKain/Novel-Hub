...
export const ageRatingService = {
  getContentWarnings: async () => {
    const res =
      await api.get<CommonResponse<ContentWarning[]>>("/content-warnings");
    return res.data;
  },

  getBookContentWarnings: async (bookId: string) => {
    const res = await api.get<CommonResponse<ContentWarning[]>>(
      `/books/${bookId}/content-warnings`,
    );
    return res.data;
  },
  ...
  getKidsModeInfo: async () => {
    const res = await api.get<CommonResponse<KidsModeInfo>>(
      "/user/kids-mode/info",
    );
    return res.data;
  },
  ...
  setKidsModePin: async (pin: string) => {
    const res = await api.post<CommonResponse<null>>("/user/kids-mode/pin", {
      pin,
    });
    return res.data;
  },
  ...
  toggleKidsMode: async (enable: boolean, pin?: string) => {
    const res = await api.post<CommonResponse<null>>("/user/kids-mode/toggle", {
      enable,
      pin,
    });
    return res.data;
