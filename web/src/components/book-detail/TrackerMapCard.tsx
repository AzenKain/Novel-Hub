...
export const TrackerMapCard: React.FC<TrackerMapCardProps> = ({
  book_id,
  title,
}) => {
  ...
  const anilistConnected = connections.some(
  ...
  if (publicSettings && publicSettings.enable_anilist_tracking === false)
    return null;
  ...
  const handleSearch = () => {
    ...
    searchMutation.mutate(query, {
      ...
      onError: (err) => {
        toast.error(
          err.message || t("trackers.search_failed", "No AniList match found"),
        );
      },
    });
  };
  ...
  const handleMap = (event: React.FormEvent) => {
    ...
    if (!trimmed) {
      toast.error(
        t(
          "trackers.enter_series_id",
          "Enter or search for an AniList series ID",
        ),
      );
      return;
    }
    mapMutation.mutate(
      ...
      {
        onSuccess: () =>
          toast.success(t("trackers.map_success", "Book linked to AniList")),
        onError: (err) =>
          toast.error(
            err.message || t("trackers.map_failed", "Failed to link book"),
          ),
      },
    );
  };
  ...
  const handleSync = () => {
    ...
    if (!Number.isInteger(value) || value < 1) {
      toast.error(
        t(
          "trackers.invalid_progress",
          "Progress must be a whole number of 1 or more",
        ),
      );
      return;
    }
    syncMutation.mutate(
      { book_id: book_id, title, progress: value },
      {
        onSuccess: () =>
          toast.success(
            t("trackers.sync_success", "Progress synced to AniList"),
          ),
        onError: (err) =>
          toast.error(
            err.message || t("trackers.sync_failed", "Failed to sync progress"),
          ),
      },
    );
  };
    ...
    <div className="space-y-3">
      ...
      {connectionsLoading ? (
        ...
        <div className="flex items-center gap-2 rounded-lg bg-base-200/60 px-3 py-2.5 text-xs text-base-content/70">
          <span>
            {t(
              "trackers.connect_in_profile_hint",
              "Connect your AniList account to link and sync this book.",
            )}
          </span>
          <Link
