...
export const AudiobookChaptersCard: React.FC<AudiobookChaptersCardProps> = ({
  book_id,
  hasAudio = false,
}) => {
  const { t } = useTranslation();
  const user = useAuthStore((state) => state.user);
  const { data: chapters, isLoading } = useAudiobookChaptersQuery(
    book_id,
    hasAudio,
  );
    ...
    upsert.mutate(
      ...
      },
      {
        onError: (err) =>
          toast.error(
            err.message ||
              t("audiobook.chapter_save_failed", "Failed to save chapter"),
          ),
      },
    );
  };
  ...
  const handleAdd = () => {
    const maxIndex =
      chapters?.reduce((m, c) => Math.max(m, c.chapter_index), -1) ?? -1;
    const key = `new-${maxIndex + 1}`;
    setDrafts((prev) => ({
      ...prev,
      [key]: {
        chapter_index: maxIndex + 1,
        title: "",
        start_sec: 0,
        end_sec: null,
      },
  ...
  const handleDelete = (ch: AudiobookChapter) => {
    remove.mutate(ch.id, {
      onError: (err) =>
        toast.error(
          err.message ||
            t("audiobook.chapter_delete_failed", "Failed to delete chapter"),
        ),
    });
  };
  ...
  const handleLookup = (e: React.FormEvent) => {
    ...
    lookup.mutate(
      { asin: trimmed },
      {
        onSuccess: () => {
          toast.success(
            t("audiobook.lookup_success", "Chapters imported from Audnexus"),
          );
          setAsin("");
        },
        onError: (err) =>
          toast.error(
            err.message ||
              t("audiobook.lookup_failed", "Audnexus lookup failed"),
          ),
      },
    );
  };

  const rows = Object.entries(drafts).sort(
    (a, b) => a[1].chapter_index - b[1].chapter_index,
  );
          ...
          {rows.map(([key, row]) => (
            <div key={key} className="flex items-center gap-2 text-sm">
              ...
              <input
                ...
                onChange={(e) =>
                  setDraft(key, { start_sec: parseSec(e.target.value, 0) ?? 0 })
                }
                onBlur={() => persist(key)}
                disabled={!canEdit}
              ...
              <input
                ...
                onChange={(e) =>
                  setDraft(key, { end_sec: parseSec(e.target.value, null) })
                }
                onBlur={() => persist(key)}
                disabled={!canEdit}
                className="input input-sm input-bordered w-24 shrink-0 text-right"
