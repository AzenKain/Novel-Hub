    ...
    >
      <div className="flex items-center justify-between border-b border-current/10 pb-2">
        ...
        <button
          type="button"
          onClick={onClose}
          className="reader-control-btn btn btn-xs btn-ghost btn-circle"
        >
          <X className="h-4 w-4" />
        </button>
      </div>
      ...
      <form onSubmit={handleSearch} className="flex gap-2">
        ...
        <button
          type="submit"
          disabled={loading}
          className="reader-action-btn btn btn-xs px-3"
        >
          {loading ? (
      ...
      <div className="overflow-y-auto flex-1 flex flex-col gap-2 divide-y divide-(--reader-ui-border)/40 pr-1">
        {results.map((res, idx) => (
          ...
          >
            <div className="font-semibold text-xs text-(--reader-ui-accent)">
              {res.chapter_title}
            </div>
            <div
