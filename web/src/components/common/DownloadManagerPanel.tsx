...
export const DownloadManagerPanel: React.FC = () => {
  ...
  const isExcludedRoute =
    ...
    ["/login", "/register", "/forgot-password", "/setup", "/activate"].includes(
      location.pathname,
    );
  ...
  const downloadingItems = activeItems.filter(
      ...
      <div className="fixed top-0 right-0 h-full w-full sm:max-w-md md:max-w-lg bg-base-100 shadow-2xl z-101 flex flex-col transform transition-transform duration-300">
        ...
        <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-6">
          ...
          <div className="rounded-xl bg-info/10 border border-info/20 p-3 flex items-start gap-2.5 text-xs text-base-content/80 leading-relaxed">
            ...
            <p>
              {t(
                "download_manager.permission_notice",
                '💡 Note: If your browser prompts for permission to download multiple files (Automatic Downloads), please click "Allow".',
              )}
            </p>
          </div>
                  ...
                  {downloadingItems.length > 0 && (
                    <div className="flex flex-col gap-3">
                      ...
                      {downloadingItems.map((item) => (
                        <div
                          key={item.id}
                          className="bg-base-200 rounded-lg p-3 flex gap-3 items-center"
                        >
                          {item.coverUrl ? (
                            <img
                              src={getMediaUrl(item.coverUrl, item.bookId)}
                              alt=""
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-2 mt-1 mb-2">
                              ...
                              {item.speed && (
                                <span className="text-xs text-base-content/70 ml-auto">
                  ...
                  {failedItems.length > 0 && (
                    <div className="flex flex-col gap-3">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex gap-1">
                          ...
                          >
                            ...
                            {t("download_manager.retry_all")}
                          </button>
                          <button
                            onClick={clearFailed}
                            className="btn btn-xs btn-ghost gap-1 text-error hover:bg-error/20 hover:text-error"
                          >
                            <Trash2 className="w-3 h-3" />{" "}
                            {t("download_manager.clear_all")}
                          </button>
                        </div>
                      </div>
                      {failedItems.map((item) => (
                        <div
                          key={item.id}
                          className="bg-error/10 border border-error/20 rounded-lg p-3 flex gap-3 items-center"
                        >
                          {item.coverUrl ? (
                            <img
                              src={getMediaUrl(item.coverUrl, item.bookId)}
                              alt=""
                              className="w-10 h-14 object-cover rounded shadow-sm opacity-80"
                            />
                          ) : (
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-2 mt-1">
                              ...
                              <span className="text-xs text-error truncate">
                                {item.error === "download_manager.interrupted"
                                  ? t(item.error)
                                  : item.error}
                              </span>
                            </div>
                  ...
                  {queuedItems.length > 0 && (
                    <div className="flex flex-col gap-3">
                      ...
                      {queuedItems.map((item) => (
                        <div
                          key={item.id}
                          className="bg-base-200/50 rounded-lg p-3 flex gap-3 items-center opacity-85 hover:opacity-100 transition-opacity"
                        >
                          {item.coverUrl ? (
                            <img
                              src={getMediaUrl(item.coverUrl, item.bookId)}
              ...
              {completedItems.length === 0 ? (
                ...
                <div className="flex flex-col gap-3">
                  ...
                  {completedItems.map((item) => (
                    <div
                      key={item.id}
                      className="bg-success/5 border border-success/20 rounded-lg p-3 flex gap-3 items-center"
                    >
                      {item.coverUrl ? (
                        <img
                          src={getMediaUrl(item.coverUrl, item.bookId)}
