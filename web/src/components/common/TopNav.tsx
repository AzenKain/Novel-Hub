...
export const TopNav: React.FC<TopNavProps> = ({
  ...
  const { data: quickResultsData, isLoading: isQuickLoading } = useQuery<
    Book[]
  >({
    ...
    <div className="navbar sticky top-0 z-30 flex-wrap gap-2 backdrop-blur-md bg-base-100/85 shadow-xs border-b border-base-200/80 px-3 sm:px-4 transition-colors duration-200">
      ...
      <div className="min-w-0 flex-1 basis-56 px-1 sm:px-2">
        <div
          ...
          className="form-control relative w-full max-w-md"
        >
            ...
            localQuery.trim().length > 0 && (
              <div className="absolute top-full left-0 right-0 mt-1.5 bg-base-100 border border-base-200 shadow-2xl rounded-2xl p-2 z-50 overflow-hidden animate-in fade-in slide-in-from-top-2">
                <div className="px-3 py-1.5 text-xs font-bold text-base-content/50 uppercase tracking-wider flex items-center justify-between">
                  <span>
                    {t("library.quick_search_results", "Quick Results")}
                  </span>
                  {isQuickLoading && (
