...
export const BookCardSkeleton: React.FC<{ compact?: boolean }> = ({
  compact,
}) => (
  <div
    className={`card card-compact bg-base-100/60 border border-base-200/60 rounded-xl overflow-hidden shadow-2xs ${compact ? "gap-1" : "gap-1.5"}`}
  >
    <div
      className={`relative ${compact ? "aspect-3/4" : "aspect-[3/4.12]"} w-full animate-shimmer rounded-t-xl`}
    />
    <div
      className={`card-body ${compact ? "p-1.5 gap-1.5" : "p-2.5 gap-2"} flex-1 flex flex-col justify-start`}
    >
