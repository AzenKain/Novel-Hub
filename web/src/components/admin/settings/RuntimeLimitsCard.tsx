...
export function RuntimeLimitsCard() {
  ...
  const { limits, limitBounds, setLimits } = useSettingsAdminStore(
    useShallow((state) => ({
      limits: state.limits,
  ...
  const isTTL = (key: keyof RuntimeLimits) =>
    key === "upload_session_ttl_seconds";
  const isSeconds = (key: keyof RuntimeLimits) => SECOND_FIELDS.includes(key);
  const displayValue = (key: keyof RuntimeLimits, value: number) =>
    isBytes(key) ? value / MEBIBYTE : isTTL(key) ? value / 60 : value;
  const backendValue = (key: keyof RuntimeLimits, value: number) =>
    Math.round(
      isBytes(key) ? value * MEBIBYTE : isTTL(key) ? value * 60 : value,
    );
  const valid = LIMIT_FIELDS.every(
    (key) =>
      limits[key] >= limitBounds.min[key] &&
      limits[key] <= limitBounds.max[key],
  );

  function save() {
    if (!limits) return;
    const payload = Object.fromEntries(
      LIMIT_FIELDS.map((key) => [SETTING_KEYS[key], limits[key]]),
    );
          ...
          <div>
            <div className="flex items-center gap-2 mb-0.5">
              ...
              <h2 className="card-title text-lg">
                {t("settings.runtime_limits")}
              </h2>
            </div>
            <p className="text-xs text-base-content/50">
              {t("settings.runtime_limits_desc")}
            </p>
          </div>
          <button
            type="button"
          ...
          <div className="col-span-full border-t border-base-200 mt-2 pt-4">
            <div className="flex flex-col gap-2">
              ...
              <div className="flex flex-wrap items-center gap-2 p-3 bg-base-200/40 border border-base-300/50 rounded-xl text-sm font-medium text-base-content/85">
                ...
                <input
                  ...
                  onChange={(event) => {
                    const value = event.currentTarget.valueAsNumber;
                    if (Number.isFinite(value))
                      setLimits({ ...limits, rate_limit_auth: value });
                  }}
                />
                ...
                <input
                  ...
                  onChange={(event) => {
                    ...
                    if (Number.isFinite(value))
                      setLimits({
                        ...limits,
                        rate_limit_auth_window_seconds: value,
                      });
                  }}
                />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 pl-1 mt-1 text-[11px] text-base-content/50">
                <div>
                  • {t("settings.limit_rate_limit_auth")}:{" "}
                  {t("settings.limit_range", {
                ...
                <div>
                  • {t("settings.limit_rate_limit_auth_window_seconds")}:{" "}
                  {t("settings.limit_range", {
