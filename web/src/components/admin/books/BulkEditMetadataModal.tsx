...
export const BulkEditMetadataModal: React.FC<BulkEditMetadataModalProps> = ({
    ...
    const initialItems: BulkEditItem[] = books.map((b) => {
      ...
      if (b.metadata_json) {
        try {
          ...
          publisher =
            meta.publisher || (meta.publishers && meta.publishers[0]) || "";
          language =
            meta.language || (meta.languages && meta.languages[0]) || "";
    ...
    switch (field) {
      ...
      case "cover":
        fieldValue = {
          ...
          pendingCover:
            sourceItem.pendingCover ||
            (sourceItem.coverPreview
              ? { type: "url", value: sourceItem.coverPreview }
              : null),
        };
        break;
    }
        ...
        switch (field) {
          ...
          case "subjects":
            if (subjectMode === "append") {
              ...
              updates.subjects = Array.from(
                new Set([...item.subjects, ...toAdd]),
              );
            } else if (subjectMode === "remove") {
              const tagToRemove = selectedTagToRemove?.trim().toLowerCase();
              if (tagToRemove) {
                updates.subjects = item.subjects.filter(
                  (s) => s.toLowerCase() !== tagToRemove,
                );
              }
            } else {
  ...
  const handleAutoSequenceSeriesIndex = () => {
    ...
    toast.success(
      t(
        "library.series_index_sequenced",
        "Sequenced series index (1 to {{count}})",
        { count: items.length },
      ),
    );
  };
  ...
  const handleSaveAll = async () => {
      ...
      try {
        ...
        if (item.pendingCover) {
          if (item.pendingCover.type === "file") {
            await bookService.updateCover(item.id, {
              cover: item.pendingCover.value as File,
            });
    ...
    if (failCount === 0) {
      toast.success(
        t(
          "library.bulk_saved_success",
          "Successfully updated metadata for {{count}} books",
          {
            count: successCount,
          },
        ),
      ...
      toast.warn(
        t(
          "library.bulk_saved_partial",
          "Updated {{success}} books, {{fail}} failed",
          {
                    ...
                    <div className="w-full md:w-36 shrink-0 flex flex-col items-center gap-3">
                      <div className="w-32 h-44 sm:w-36 sm:h-50 rounded-2xl bg-base-300 border border-base-300/80 overflow-hidden shadow-md relative group flex items-center justify-center">
                        {item.coverPreview ? (
                          <img
                            src={
                              item.coverPreview.startsWith("blob:") ||
                              item.coverPreview.startsWith("http")
                                ? item.coverPreview
                                : getMediaUrl(
                                    item.coverPreview,
                                    item.id,
                          ...
                          <button
                            ...
                            onClick={() => {
                              setCoverUrlModal({
                                isOpen: true,
                                bookIndex: index,
                                url: item.coverPreview?.startsWith("http")
                                  ? item.coverPreview
                                  : "",
                              });
                            }}
                        ...
                        <div className="flex flex-col gap-2.5">
                          <div className="flex gap-2 w-full">
                            <input
                              ...
                              value={item.tagInput}
                              onChange={(e) =>
                                updateItem(index, { tagInput: e.target.value })
                              }
                              onKeyDown={(e) => {
                                if (e.key === "Enter" || e.key === ",") {
                                  e.preventDefault();
                                  const val = item.tagInput
                                    .trim()
                                    .replace(/^,+|,+$/g, "");
                              ...
                              {item.subjects.map((sub, sIdx) => (
                                ...
                                >
                                  ...
                                  <button
                                    type="button"
                                    onClick={() => {
                                      const next = item.subjects.filter(
                                        (_, i) => i !== sIdx,
                                      );
              ...
              {syncState.field === "subjects" && (
                <div className="space-y-3 p-4 bg-base-200/50 rounded-2xl border border-base-200">
                  <span className="text-xs font-bold text-base-content/80 block uppercase tracking-wider">
                    {t(
                      "library.subject_operation_mode",
                      "Tag / Subject apply mode",
                    )}
                    :
                  </span>
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                    ...
                    >
                      <div className="text-xs font-bold">
                        {t("library.tag_mode_append")}
                      </div>
                      <div className="text-[11px] font-normal opacity-70 mt-0.5">
                        {t("library.tag_mode_append_desc")}
                      </div>
                    </button>
                    <button
                      type="button"
                    ...
                    >
                      <div className="text-xs font-bold">
                        {t("library.tag_mode_remove")}
                      </div>
                      <div className="text-[11px] font-normal opacity-70 mt-0.5">
                        {t("library.tag_mode_remove_desc")}
                      </div>
                    </button>
                    <button
                      type="button"
                    ...
                    >
                      <div className="text-xs font-bold">
                        {t("library.tag_mode_replace")}
                      </div>
                      <div className="text-[11px] font-normal opacity-70 mt-0.5">
                        {t("library.tag_mode_replace_desc")}
                      </div>
                    </button>
                  </div>
                    ...
                    <div className="mt-3 pt-3 border-t border-base-300 space-y-2">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setSyncState((prev) => ({
                            ...prev,
                            selectedTagToRemove: e.target.value,
                          }))
                        }
                        placeholder={t(
                          "library.or_type_tag_to_remove",
                          "Or type a tag name to remove...",
                        )}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-xs text-base-content/60 truncate mt-0.5">
                          {item.author ||
                            ...
                            t("book.author_unknown", "Unknown author")}
                        </div>
                      </div>
                    </label>
                  );
            ...
            >
              <div>
                ...
                <span className="text-[11px] text-base-content/50 mt-1 block">
                  {t(
                    "admin.cover_url_hint",
                    "Supports direct image formats (JPG, PNG, WEBP, GIF)",
                  )}
                </span>
              </div>
