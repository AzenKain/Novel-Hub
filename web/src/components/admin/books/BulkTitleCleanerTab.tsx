...
interface BulkTitleCleanerTabProps {
  items: BulkEditItem[];
  onApplyChanges: (changes: Array<{ id: string; title: string; author: string }>) => void;
}
...
export const BulkTitleCleanerTab: React.FC<BulkTitleCleanerTabProps> = ({
  ...
  // Local working state for each book in cleaner
  const [cleanedState, setCleanedState] = useState<
    Record<string, { title: string; author: string; enabled: boolean }>
  >(() => {
    const init: Record<string, { title: string; author: string; enabled: boolean }> = {};
    items.forEach((it) => {
      init[it.id] = {
        title: it.title,
        author: it.author,
  ...
  useEffect(() => {
    setCleanedState((prev) => {
      const next: Record<string, { title: string; author: string; enabled: boolean }> = {};
      items.forEach((it) => {
        next[it.id] = {
  ...
  const [regexTarget, setRegexTarget] = useState<'title' | 'author' | 'both'>('title');

  // 1-Click Preset Handlers
  const applyPreset = (
    cleanerFn: (title: string, author: string) => { title: string; author: string }
  ) => {
    setCleanedState((prev) => {
      const next = { ...prev };
      let changedCount = 0;
      ...
      toast.info(t('library.cleaner_preset_applied', 'Updated {{count}} books', { count: changedCount }));
      return next;
    });
  };
      ...
      setCleanedState((prev) => {
        ...
        Object.keys(next).forEach((id) => {
          ...
          if (regexTarget === 'title' || regexTarget === 'both') {
            newTitle = newTitle.replace(reg, regexReplace).replace(/\s+/g, ' ').trim();
          }
          if (regexTarget === 'author' || regexTarget === 'both') {
            newAuthor = newAuthor.replace(reg, regexReplace).replace(/\s+/g, ' ').trim();
          }
        ...
        toast.info(t('library.cleaner_regex_applied', 'Regex updated {{count}} books', { count }));
        return next;
      });
    } catch (err) {
      toast.error(t('library.cleaner_regex_invalid', 'Invalid Regular Expression'));
    }
  };
  ...
  const handleApplyToBooks = () => {
    ...
    toast.success(
      t('library.cleaner_applied_success', 'Applied cleaned titles and authors to {{count}} books', {
    ...
    <div className="flex flex-col gap-4 p-4">
      ...
      <div className="bg-base-200/50 p-3.5 rounded-2xl border border-base-300">
        ...
        <div className="mt-3 pt-3 border-t border-base-300 flex flex-wrap items-center gap-2 text-xs">
          ...
          <input
            type="text"
            placeholder={t('library.cleaner_regex_pattern', 'Pattern (e.g. ^Chương \\d+:\\s*)')}
            value={regexPattern}
          ...
          >
            <option value="title">{t('library.title_only', 'Title Only')}</option>
            <option value="author">{t('library.author_only', 'Author Only')}</option>
      ...
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-base-content/60">
            {t('library.cleaner_selected_count', '{{count}} books enabled for cleanup', {
              count: Object.values(cleanedState).filter((v) => v.enabled).length,
            })}
          </span>
        </div>
        ...
        <table className="table table-xs table-pin-rows">
          <thead>
            <tr className="bg-base-200/80">
              <th className="w-8"></th>
              <th className="w-5/12">{t('library.original_metadata', 'Original Title & Author')}</th>
              <th className="w-8 text-center"></th>
              <th className="w-6/12">{t('library.cleaned_metadata', 'Cleaned Preview (Editable)')}</th>
            </tr>
          </thead>
          <tbody>
            {filteredItems.map((item) => {
              ...
              return (
                <tr key={item.id} className={current.enabled ? 'hover:bg-base-200/40' : 'opacity-40'}>
                  <td>
                    <input
                      type="checkbox"
                      ...
                      onChange={(e) =>
                        setCleanedState((p) => ({
                          ...p,
                          [item.id]: { ...p[item.id], enabled: e.target.checked },
                        }))
                      }
                  ...
                  <td>
                    <div className="font-semibold text-xs leading-snug line-clamp-1">{item.original.title}</div>
                    <div className="text-[11px] text-base-content/50">
                      {item.original.author_name || t('common.unknown_author', 'Unknown')}
                    </div>
                  </td>
                  <td className="text-center">
                    <ArrowRight className={`h-3.5 w-3.5 mx-auto ${isModified ? 'text-primary' : 'text-base-content/20'}`} />
                  </td>
                  <td>
                    <div className="flex flex-col gap-1">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setCleanedState((p) => ({
                            ...p,
                            [item.id]: { ...p[item.id], author: e.target.value },
                          }))
                        }
