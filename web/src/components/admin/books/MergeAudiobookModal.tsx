...
const VolumeEnvelope: React.FC<{
  ...
  const calcGainFromY = useCallback(
    (clientY: number) => {
...
/* ── Main Modal ────────────────────────────────────────────────────── */
export const MergeAudiobookModal: React.FC<MergeAudiobookModalProps> = ({
  open,
  book_id,
  title,
  files,
  ...
  const [decodedBuffers, setDecodedBuffers] = useState<
  ...
  const handleTimeUpdate = () => {
    ...
    const localPlayhead =
      audioRef.current.currentTime - activeRange.segment.startOffset;
    ...
    if (fileOffset >= activeRange.segment.endOffset - 0.08) {
      ...
      if (nextIndex < segmentRanges.length) {
        ...
        const nextSrc = bookService.getDownloadUrl(
          book_id,
          nextRange.segment.fileId,
        );
  ...
  const handleScrub = (newGlobalTime: number) => {
    ...
    if (audioRef.current) {
      const activeRange =
        segmentRanges.find(
          (r) => clampedTime >= r.start && clampedTime <= r.end,
        ) || segmentRanges[0];
      if (activeRange && activeRange.segment.duration > 0) {
        const newSrc = bookService.getDownloadUrl(
          book_id,
          activeRange.segment.fileId,
        );
  ...
  const togglePlay = () => {
    ...
    if (playing) {
      ...
      if (activeRange) {
        ...
        const targetTime =
          currentTime - activeRange.start + activeRange.segment.startOffset;
        const isSameSrc = audioRef.current.src.endsWith(newSrc);
  ...
  const handleSplitAtPlayhead = () => {
    ...
    if (
      localSplitTime <= 0.5 ||
      localSplitTime >= activeRange.segment.duration - 0.5
    ) {
      toast.error(
        t(
          "audiobook.cannot_split_close",
          "Cannot split too close to track boundaries (minimum 0.5s from edge)",
        ),
      );
      return;
    }
    ...
    const baseName =
      activeRange.segment.fileName.substring(
        0,
        activeRange.segment.fileName.lastIndexOf("."),
      ) || activeRange.segment.fileName;
    ...
    const seg1: TimelineSegment = {
      ...
      sizeBytes: Math.floor(
        activeRange.segment.sizeBytes *
          (localSplitTime / activeRange.segment.duration),
      ),
    ...
    const seg2: TimelineSegment = {
      ...
      sizeBytes: Math.floor(
        activeRange.segment.sizeBytes *
          ((activeRange.segment.duration - localSplitTime) /
            activeRange.segment.duration),
      ),
    ...
    toast.success(
      t("audiobook.timeline_split_success", "Timeline split locally!"),
    );
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (segments.length < 2) {
      toast.error(
        t(
          "audiobook.merge_need_two",
          "Timeline must contain at least 2 segments to merge",
        ),
      );
      return;
    }
    if (segments.some((s) => s.duration <= 0)) {
      toast.error(
        t(
          "audiobook.merge_not_ready",
          "Still decoding audio — wait until all segments have a duration",
        ),
      );
      return;
    }
    ...
    toast.info(
      t("audiobook.submitting_merge", "Submitting merge request to server..."),
    );
    merge.mutate(
      {
      ...
      },
      {
        ...
        onError: (err) => {
          toast.error(
            err.message || t("audiobook.merge_failed", "Failed to start merge"),
          );
        },
      },
    );
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <label className="text-xs font-bold uppercase opacity-60 block">
                {t("audiobook.timeline", "Audio Timeline")}
              </label>
              {/* Zoom controls */}
              <div className="flex items-center gap-2 text-xs">
                ...
                <span className="font-mono opacity-60 w-10 text-right">
            ...
            <div className="relative border border-base-300 rounded-2xl bg-base-200/20 overflow-x-auto overflow-y-hidden shadow-xs">
              ...
              >
                {segmentRanges.map((range, index) => {
                  ...
                  const localPlayheadPct =
                    isActive && range.segment.duration > 0
                      ? (currentTime - range.start) / range.segment.duration
                      : 0;
                  const isDragSource =
                  ...
                  const isActiveClass = isActive
                    ? "ring-2 ring-primary/50 shadow-md scale-[0.99] z-10"
                    : "";
                  const isDragOver =
                    dragOverIndex === index && dragState.current?.active;
                  const dragOverBorderClass =
                    isDragOver && dragState.current
                      ? dragState.current.sourceIndex < index
                      ...
                      >
                        ...
                        <div className="flex items-center justify-between text-[9px] font-bold text-base-content/60 px-2 pt-1.5 pb-0.5 gap-1">
                          ...
                          <div className="flex items-center gap-1.5 shrink-0">
                            ...
                            <span className="font-mono opacity-80">
                              {formatTime(range.segment.duration)}
                            </span>
                          </div>
                        </div>
                        ...
                        <div className="flex-1 relative px-1 pb-1">
                          ...
                          <VolumeEnvelope
                            ...
                            onGainChange={(g) =>
                              updateSegmentGain(range.segment.id, g)
                            }
                          />
                        </div>
                      </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="label label-text font-semibold">
                {t("audiobook.merged_title", "Merged title")}
              </label>
              <input
                type="text"
                value={mergedTitle}
            ...
            <label className="label label-text font-semibold">
              ...
              {orderedFiles.length > 0 && (
                <span className="badge badge-ghost badge-sm">
                  {selected.size}/{orderedFiles.length}
                </span>
              )}
            </label>
            {orderedFiles.length === 0 ? (
              <p className="text-sm opacity-60">
                {t(
                  "audiobook.no_mergeable_files",
                  "No audio files on this book",
                )}
              </p>
            ) : (
              <div className="max-h-52 overflow-y-auto space-y-1.5 rounded-box border border-base-300 p-2">
                {orderedFiles.map((f, index) => {
                  return (
                    <div
                      key={f.id}
                      className="flex items-center justify-between gap-2 p-2 rounded-lg hover:bg-base-200 border border-base-200 bg-base-100/50"
                    >
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        ...
                        <button
                          ...
                          title={t(
                            "audiobook.remove_from_timeline",
