...
export const BulkSendEmailModal: React.FC<BulkSendEmailModalProps> = ({
  ...
  return (
    <dialog className="modal modal-open z-50">
      <div className="modal-box max-w-2xl w-11/12 p-0 rounded-2xl shadow-2xl border border-base-300 bg-base-100 flex flex-col max-h-[90dvh] sm:max-h-[88vh] overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        {/* Window Header (Sticky with clear X button) */}
        <div className="sticky top-0 z-30 flex items-center justify-between px-5 py-3.5 bg-base-200/95 backdrop-blur-md border-b border-base-200 shrink-0">
          ...
          <button
            ...
            className="btn btn-sm btn-circle btn-ghost -mr-1 text-base-content/70 hover:text-base-content"
        ...
        <form id="bulk-send-email-form" onSubmit={handleSend} className="flex-1 overflow-y-auto min-h-0 flex flex-col">
          {/* Recipients Row */}
          ...
          {/* Editor / Preview Area */}
          <div className="flex-1 shrink-0 bg-base-100 flex flex-col min-h-55 w-full" style={{ flex: "1 0 auto" }}>
            {activeTab === "write" ? (
              <textarea
                ref={textareaRef}
                required
                ...
                className="w-full p-4 sm:p-5 text-sm leading-relaxed text-base-content placeholder:text-base-content/30 bg-transparent focus:outline-none resize-none font-sans overflow-hidden block min-h-55"
              />
            ) : (
              <div className="p-4 sm:p-5 min-h-55 text-sm leading-relaxed select-text flex-1 shrink-0" style={{ flex: "1 0 auto" }}>
                {previewBody.trim() ? (
        ...
        {/* Footer (Pinned, solid background, never overlaps content) */}
        <div className="shrink-0 px-4 sm:px-6 py-3 bg-base-100 border-t border-base-200 flex items-center justify-between gap-3 flex-wrap z-20">
          {/* Meta stats */}
      ...
      <form method="dialog" className="modal-backdrop" onClick={onClose}>
        <button type="button" disabled={sendEmailMutation.isPending}>
