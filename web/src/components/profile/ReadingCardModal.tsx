...
export const ReadingCardModal: React.FC<ReadingCardModalProps> = ({
  ...
  const defaultQuote = t('analytics.card_quote', 'A reader lives a thousand lives.');
  ...
  const stats = useMemo(() => {
    ...
    const prefix = period === 'year' ? `${currentYear}-` : `${currentYear}-${monthStr}-`;
  ...
  const peakDay = useMemo(() => {
    ...
    let maxDate = '';
    Object.entries(heatmapData).forEach(([date, item]) => {
      if ((prefix === '' || date.startsWith(prefix)) && item && item.words > maxWords) {
        maxWords = item.words;
        maxDate = date;
      }
    });
  ...
  const readerLevel = useMemo(() => {
    const w = stats.words;
    if (w >= 500000) return { title: t('analytics.rank_grandmaster', 'Grandmaster Reader'), icon: '🏆' };
    if (w >= 200000) return { title: t('analytics.rank_scholar', 'Erudite Scholar'), icon: '🌟' };
    if (w >= 100000) return { title: t('analytics.rank_bookworm', 'Avid Bookworm'), icon: '📖' };
    if (w >= 30000) return { title: t('analytics.rank_explorer', 'Knowledge Explorer'), icon: '🚀' };
    return { title: t('analytics.rank_novice', 'Novice Reader'), icon: '🌱' };
  }, [stats.words, t]);

  const avgWordsPerDay = stats.activeDays > 0 ? Math.round(stats.words / stats.activeDays) : 0;
  ...
  const themeStyles = useMemo(() => {
    switch (theme) {
      case 'cyberpunk':
        return {
          container: 'bg-[#090a0f] text-[#f1f5f9] border border-amber-400/40 shadow-2xl',
          ...
          levelBadge: 'bg-amber-400/15 text-amber-300 border border-amber-400/30',
        };
      case 'sepia':
        return {
          container: 'bg-[#fcf8f2] text-[#2c1810] border-2 border-[#c5a880] shadow-2xl font-serif',
      ...
      default:
        return {
          container: 'bg-gradient-to-br from-slate-950 via-indigo-950 to-purple-950 text-slate-100 border border-indigo-500/30 shadow-2xl',
          heroNumber: 'text-transparent bg-clip-text bg-gradient-to-r from-cyan-300 via-indigo-300 to-fuchsia-300',
          badge: 'bg-indigo-500/20 text-indigo-300 border border-indigo-500/40',
          statBox: 'bg-white/[0.05] border border-white/10 backdrop-blur-sm text-slate-200',
          accentText: 'text-indigo-400',
          subText: 'text-slate-400',
          borderCol: 'border-white/10',
          tagBadge: 'bg-fuchsia-500/15 text-fuchsia-300 border border-fuchsia-500/30',
          barActive: 'bg-gradient-to-t from-indigo-500 to-fuchsia-400',
          barInactive: 'bg-white/10',
          levelBadge: 'bg-indigo-500/20 text-indigo-200 border border-indigo-500/30',
        };
    }
  }, [theme]);
    ...
    try {
      ...
      canvas.toBlob(async (blob) => {
        ...
        try {
          ...
          setCopied(true);
          toast.success(t('analytics.card_copied_toast', 'Card copied to clipboard!'));
  ...
  const avatarUrl = user?.avatar_url ? getMediaUrl(user.avatar_url, undefined, user.updated_at) : null;
      ...
      <div className="modal-box max-w-5xl p-4 sm:p-6 bg-base-100 border border-base-300">
        <div className="flex items-center justify-between pb-3 border-b border-base-200">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="font-bold text-lg">{t('analytics.card_title', 'Reading Wrapped')}</h3>
              <p className="text-xs text-base-content/60">{t('analytics.card_subtitle', 'Share your reading achievements')}</p>
            </div>
          </div>
          <button type="button" onClick={onClose} className="btn btn-ghost btn-sm btn-square" aria-label={t('common.close', 'Close')}>
            <X className="h-4 w-4" />
          </button>
        </div>
        ...
        <div className="mt-4 grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          <div className="lg:col-span-5 flex flex-col gap-4">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {(['aurora', 'cyberpunk', 'sepia', 'eink'] as CardTheme[]).map((thm) => (
                  <button
                    key={thm}
                    type="button"
                  ...
                  >
                    <div className={`w-3 h-3 rounded-full border ${thm === 'aurora' ? 'bg-indigo-600' : thm === 'cyberpunk' ? 'bg-amber-400' : thm === 'sepia' ? 'bg-[#c5a880]' : 'bg-black border-white'}`} />
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <label className="text-xs font-bold uppercase tracking-wider text-base-content/60 block">
                  {t('analytics.card_custom_quote_label', 'Custom Quote / Message')}
                </label>
          ...
          <div className="lg:col-span-7 flex justify-center items-center overflow-auto p-4 bg-base-200/60 rounded-2xl border border-base-300 min-h-[440px]">
            <div
              ...
              style={{
                width: ratio === 'story' ? '380px' : ratio === 'wide' ? '540px' : '400px',
                minHeight: ratio === 'story' ? '640px' : ratio === 'wide' ? '340px' : '400px',
              }}
            >
              {/* Header */}
              <div>
                <div className="flex items-center justify-between gap-3">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      <div className="font-bold text-sm leading-tight truncate max-w-[160px]">{displayName}</div>
                      <div className={`text-[11px] ${themeStyles.subText}`}>NovelHub Reader</div>
                    </div>
                  </div>
                  <span className={`text-[11px] px-2.5 py-1 rounded-full font-medium tracking-wide uppercase ${themeStyles.badge}`}>
                    {stats.periodLabel}
                  </span>
                </div>
                ...
                {ratio === 'wide' ? (
                  <div className="grid grid-cols-2 gap-4 mt-4">
                    ...
                    <div className="flex flex-col justify-between gap-3">
                      <div>
                        <div className={`text-xs uppercase tracking-widest font-bold ${themeStyles.subText}`}>
                          {t('analytics.words_read', 'Words Read')}
                        </div>
                        <div className={`text-3xl font-black tracking-tight mt-0.5 ${themeStyles.heroNumber}`}>
                          {stats.words.toLocaleString()}
                        </div>
                        <div className="flex items-center gap-2 mt-1">
                          <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold flex items-center gap-1 ${themeStyles.levelBadge}`}>
                      ...
                      <div className="grid grid-cols-3 gap-1.5">
                        <div className={`p-2 rounded-xl flex flex-col items-center text-center ${themeStyles.statBox}`}>
                          <Clock className={`h-3.5 w-3.5 mb-0.5 ${themeStyles.accentText}`} />
                          <div className="font-bold text-xs">
                            {hoursRead > 0 ? `${hoursRead}h ${remainingMins}m` : `${stats.minutes}m`}
                          </div>
                          <div className={`text-[9px] ${themeStyles.subText}`}>{t('analytics.card_hours_read', 'Time Read')}</div>
                        </div>

                        <div className={`p-2 rounded-xl flex flex-col items-center text-center ${themeStyles.statBox}`}>
                          <Flame className={`h-3.5 w-3.5 mb-0.5 ${themeStyles.accentText}`} />
                          <div className="font-bold text-xs">{stats.streak} {t('analytics.days', 'd')}</div>
                          <div className={`text-[9px] ${themeStyles.subText}`}>{t('analytics.card_streak', 'Best Streak')}</div>
                        </div>

                        <div className={`p-2 rounded-xl flex flex-col items-center text-center ${themeStyles.statBox}`}>
                          <Calendar className={`h-3.5 w-3.5 mb-0.5 ${themeStyles.accentText}`} />
                          <div className="font-bold text-xs">{stats.activeDays} {t('analytics.days', 'd')}</div>
                          <div className={`text-[9px] ${themeStyles.subText}`}>{t('analytics.card_active_days', 'Active Days')}</div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col justify-between gap-2.5">
                      {/* 7-Day Rhythm */}
                      <div className={`p-2.5 rounded-xl ${themeStyles.statBox} flex flex-col gap-1`}>
                        <div className="flex items-center justify-between">
                          <span className={`text-[9px] uppercase font-bold tracking-wider ${themeStyles.subText} flex items-center gap-1`}>
                            <TrendingUp className="w-3 h-3 text-primary" />
                            {t('analytics.card_reading_rhythm', '7-Day Reading Rhythm')}
                          </span>
                          <span className={`text-[9px] font-semibold ${themeStyles.accentText}`}>
                            {last7Days.reduce((acc, b) => acc + b.words, 0).toLocaleString()} {t('analytics.words', 'words')}
                          </span>
                        </div>
                        <div className="flex items-end justify-between gap-1 h-10 pt-1 px-1">
                          {last7Days.map((b, idx) => {
                            const heightPercent = max7DaysWords > 0 ? Math.max(12, Math.round((b.words / max7DaysWords) * 100)) : 12;
                            const isToday = idx === 6;
                            return (
                              <div key={b.date} className="flex-1 flex flex-col items-center gap-0.5 h-full justify-end">
                                <div
                                  className={`w-full rounded-xs transition-all ${
                                    b.words > 0 ? (isToday ? 'bg-primary shadow-xs' : themeStyles.barActive) : themeStyles.barInactive
                                  }`}
                                  style={{ height: `${heightPercent}%` }}
                                />
                                <span className={`text-[8px] font-medium leading-none ${isToday ? 'text-primary font-bold' : themeStyles.subText}`}>
                                  {b.day}
                                </span>
                              </div>
                            );
                      ...
                      {/* Highlights */}
                      {(topGenres.length > 0 || topAuthors.length > 0 || topFormats.length > 0) && (
                        <div className="flex flex-wrap gap-1">
                          {topGenres.slice(0, 2).map((g) => (
                            <span key={g} className={`text-[10px] px-2 py-0.5 rounded-md flex items-center gap-1 ${themeStyles.tagBadge}`}>
                              <Bookmark className="h-2.5 w-2.5" /> {g}
                            </span>
                          ))}
                          {topAuthors.slice(0, 1).map((a) => (
                            <span key={a} className={`text-[10px] px-2 py-0.5 rounded-md flex items-center gap-1 ${themeStyles.tagBadge}`}>
                              <BookOpen className="h-2.5 w-2.5" /> {a}
                            </span>
                          ))}
                          {topFormats.slice(0, 1).map((f) => (
                            <span key={f} className={`text-[10px] px-2 py-0.5 rounded-md flex items-center gap-1 ${themeStyles.tagBadge}`}>
                  ...
                  <>
                    ...
                    <div className="mt-5">
                      ...
                      <div className={`text-4xl sm:text-5xl font-black tracking-tight mt-0.5 ${themeStyles.heroNumber}`}>
                        {stats.words.toLocaleString()}
                      </div>
                      <div className="flex items-center gap-2 mt-2">
                        <span className={`text-[11px] px-2.5 py-0.5 rounded-full font-bold flex items-center gap-1.5 ${themeStyles.levelBadge}`}>
                          <span>{readerLevel.icon}</span>
                          <span>{readerLevel.title}</span>
                        </span>
                        {avgWordsPerDay > 0 && (
                          <span className={`text-[10px] ${themeStyles.subText}`}>
                    ...
                    <div className="grid grid-cols-3 gap-2 mt-4">
                      <div className={`p-2.5 rounded-xl flex flex-col items-center text-center ${themeStyles.statBox}`}>
                        <Clock className={`h-4 w-4 mb-1 ${themeStyles.accentText}`} />
                        <div className="font-bold text-sm">
                          {hoursRead > 0 ? `${hoursRead}h ${remainingMins}m` : `${stats.minutes}m`}
                        </div>
                        <div className={`text-[10px] ${themeStyles.subText}`}>{t('analytics.card_hours_read', 'Time Read')}</div>
                      </div>

                      <div className={`p-2.5 rounded-xl flex flex-col items-center text-center ${themeStyles.statBox}`}>
                        <Flame className={`h-4 w-4 mb-1 ${themeStyles.accentText}`} />
                        <div className="font-bold text-sm">{stats.streak} {t('analytics.days', 'd')}</div>
                        <div className={`text-[10px] ${themeStyles.subText}`}>{t('analytics.card_streak', 'Best Streak')}</div>
                      </div>

                      <div className={`p-2.5 rounded-xl flex flex-col items-center text-center ${themeStyles.statBox}`}>
                        <Calendar className={`h-4 w-4 mb-1 ${themeStyles.accentText}`} />
                        <div className="font-bold text-sm">{stats.activeDays} {t('analytics.days', 'd')}</div>
                        <div className={`text-[10px] ${themeStyles.subText}`}>{t('analytics.card_active_days', 'Active Days')}</div>
                      </div>
                    </div>

                    {/* Habit Insights Row (Story Mode only or when peakDay exists) */}
                    {(ratio === 'story' || peakDay) && (
                      <div className={`mt-3 p-2.5 rounded-xl ${themeStyles.statBox} flex items-center justify-around text-center gap-2`}>
                        <div>
                          <div className={`text-[9px] uppercase font-bold tracking-wider ${themeStyles.subText}`}>
                            {t('analytics.card_daily_pace', 'Daily Pace')}
                          </div>
                          <div className="font-bold text-xs mt-0.5">
                            {avgWordsPerDay > 0 ? `~${avgWordsPerDay.toLocaleString()} ${t('analytics.words_per_day', 'words/day')}` : '--'}
                          </div>
                        </div>
                        ...
                        <div>
                          ...
                          <div className="font-bold text-xs mt-0.5">
                            {peakDay ? `${peakDay.date} (${peakDay.words.toLocaleString()})` : '--'}
                          </div>
                        </div>
                      </div>
                    )}
                    ...
                    {ratio === 'story' && (
                      <div className={`mt-3 p-3 rounded-2xl ${themeStyles.statBox} flex flex-col gap-1.5`}>
                        <div className="flex items-center justify-between">
                          <span className={`text-[10px] uppercase font-bold tracking-wider ${themeStyles.subText} flex items-center gap-1.5`}>
                            <TrendingUp className="w-3.5 h-3.5 text-primary" />
                            {t('analytics.card_reading_rhythm', '7-Day Reading Rhythm')}
                          </span>
                          <span className={`text-[10px] font-semibold ${themeStyles.accentText}`}>
                            {last7Days.reduce((acc, b) => acc + b.words, 0).toLocaleString()} {t('analytics.words', 'words')}
                          </span>
                        </div>
                        <div className="flex items-end justify-between gap-1.5 h-12 pt-1 px-1">
                          {last7Days.map((b, idx) => {
                            ...
                            const isToday = idx === 6;
                            return (
                              <div key={b.date} className="flex-1 flex flex-col items-center gap-1 h-full justify-end">
                                <div
                                ...
                                />
                                <span className={`text-[9px] font-medium leading-none ${isToday ? 'text-primary font-bold' : themeStyles.subText}`}>
                                  {b.day}
                                </span>
                              </div>
                            );
                    ...
                    {/* Highlights */}
                    {(topGenres.length > 0 || topAuthors.length > 0 || topFormats.length > 0) && (
                      <div className={`mt-3 pt-3 border-t ${themeStyles.borderCol} flex flex-col gap-2`}>
                        <div className={`text-[10px] uppercase font-bold tracking-wider ${themeStyles.subText}`}>
                          {t('analytics.card_highlights', 'Reading Highlights')}
                        </div>
                        <div className="flex flex-wrap gap-1.5">
                          {topGenres.map((g) => (
                            <span key={g} className={`text-[11px] px-2 py-0.5 rounded-lg flex items-center gap-1 ${themeStyles.tagBadge}`}>
                              <Bookmark className="h-3 w-3" /> {g}
                            </span>
                          ))}
                          {topAuthors.map((a) => (
                            <span key={a} className={`text-[11px] px-2 py-0.5 rounded-lg flex items-center gap-1 ${themeStyles.tagBadge}`}>
                              <BookOpen className="h-3 w-3" /> {a}
                            </span>
                          ))}
                          {topFormats.map((f) => (
                            <span key={f} className={`text-[11px] px-2 py-0.5 rounded-lg flex items-center gap-1 ${themeStyles.tagBadge}`}>
              ...
              {/* Footer */}
              <div className={`mt-5 pt-3.5 border-t ${themeStyles.borderCol} flex items-center ${showLogo ? 'justify-between' : 'justify-center'} gap-2`}>
                {showLogo && (
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <span className="font-black text-xs tracking-wider uppercase">{siteTitle}</span>
                  </div>
                )}
                {customQuote.trim() && (
                  <div className={`text-[10px] italic ${themeStyles.subText} ${showLogo ? 'text-right' : 'text-center'} truncate max-w-[220px]`}>
                    "{customQuote.trim()}"
                  </div>
                )}
              </div>
