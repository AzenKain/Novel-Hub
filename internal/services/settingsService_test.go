...
func TestRuntimeLimitsFromRaw(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "valid rate limits", raw: map[string]any{
			"limits.rate_limit_api":                 float64(constants.MinRuntimeRateLimitAPI),
			"limits.rate_limit_api_window_seconds":  float64(constants.HardMaxRateLimitAPIWindowSeconds),
		...
		{name: "rate limit below floor rejected", raw: map[string]any{"limits.rate_limit_api": float64(constants.MinRuntimeRateLimitAPI - 1)}},
		...
		{name: "zero rate limit window rejected", raw: map[string]any{"limits.rate_limit_api_window_seconds": float64(0)}},
