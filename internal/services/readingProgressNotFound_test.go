...
// A book that has never been opened must produce ErrNotFound, not a nil entity with no error:
// the controller maps ErrNotFound to 404, and Kobo's readingState branches on it to decide
// whether the device gets a ReadingState at all. While the repository answered (nil, nil), both
