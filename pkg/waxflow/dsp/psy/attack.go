...
// AttackDetector flags transient energy jumps for window switching.
...
// NewAttackDetector builds a detector firing when a sub-window holds ratio times the preceding level.
...
// Scan examines one block split into sub equal sub-windows and reports the first attack and which sub-window holds it.
