...
// SampleRate is Opus's fixed decode rate; every stream decodes to 48 kHz regardless of the OpusHead input rate (RFC 7845).
const SampleRate = 48000

// SeekPreroll is the decoder convergence time RFC 7845 section 4.2 recommends: 80 ms at Opus's fixed 48 kHz.
const SeekPreroll = 3840

// SetPreSkip returns a copy of an OpusHead header with its pre-skip set to n samples.
...
// Config is the stream configuration from the OpusHead identification header (RFC 7845 section 5.1).
type Config struct {
	Channels int
	PreSkip  int
	Gain     int16
...
// tocTable maps the 5-bit TOC config to its mode, bandwidth, and frame size (RFC 6716 Table 2).
...
// PacketSamples returns the number of 48 kHz samples a packet decodes to, derived from its TOC byte and frame count alone (RFC 6716 section 3.1).
...
// splitPacket parses a packet's TOC and framing into individual frames (RFC 6716 section 3.2).
