...
- **Offline Reading & Text-to-Speech**: Save books to the browser for offline reading (PWA, `vite-plugin-pwa`), plus a text-to-speech audio reader. Both are permission-gated (`book.offline`, `book.tts`).
...
- **OPDS Server**: Built-in OPDS 1.2 and 2.0 catalogs at `/api/opds/v1` and `/api/opds/v2/catalog` (HTTP Basic auth and guest access controls) for mobile reader apps such as KOReader, Calibre, Moon+ Reader and Thorium.
...
- **Email Delivery (SMTP)**: Admin-configured SMTP with STARTTLS / implicit TLS, connection testing from the UI, and SSRF-safe dialling. Powers Send-to-Kindle (`book.send_email`) and all account email.
...
- **High Performance Architecture**: Powered by `theine-go` in-memory RAM caching (Cache-by-IDs pattern), `singleflight` for thundering-herd protection, and 22 purpose-built SQLite indexes for sub-millisecond query execution.
...
- **Library Management**: Automatic file scanning, cover extraction, metadata editing, tag/author filtering, and SQLite FTS5 full-text search. A watched inbox (`DATA_DIR/inbox/<libraryID>/`) imports anything dropped into it.
