...
- **High Performance Architecture**: Powered by `theine-go` in-memory RAM caching (Cache-by-IDs pattern for search & random shelves), `singleflight` for thundering-herd protection, and 64 purpose-built SQLite indexes for sub-millisecond query execution. The three FTS5 tables are rowid-aligned with their source tables, so an FTS write is a rowid seek rather than a full scan of the index.
