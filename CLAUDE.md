...
- **Shared Packages (`pkg/`)**:
  - `pkg/apperrors`: All custom errors (`ErrBadRequest`, `ErrNotFound`, etc.). Controllers call `apperrors.HandleError(c, err)`.
  - `pkg/convert`: ID parsing & null conversion helpers.
  - `pkg/constants`: Cache TTLs and centralized cache key constants in `pkg/constants/cache_keys.go`. NEVER hardcode raw cache key strings.
...
- **System Operations & Environment**:
  - Log rotation (`LOG_MAX_SIZE_MB`, `LOG_MAX_FILES`), Online SQLite backup (`sqliteSnapshot`), staged database restore validation, and auto-restart integration (`RESTORE_AUTO_RESTART`).
